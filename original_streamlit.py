...
def get_openai_client(api_base: str):
    """Return an OpenAI-compatible client for the *current* api_base"""
    return OpenAI(base_url=api_base, api_key="lm_studio")
...
with st.sidebar:
    ...
    if selected_model != st.session_state["model_name"]:
        ...
        # Trigger model loading in LM Studio with a test request
        with st.spinner(f"Loading {selected_model}..."):
            try:
                client = get_openai_client(st.session_state["api_base"])
                ...
                st.warning(f"Note: Model may still be loading. Error: {str(e)}")
